module.exports = {
  ...
  execute: async (client, msg, args) => {
    ...
    if (
      (args[0] === 'off' && queue.filters.length) ||
      (args[0] === 'none' && queue.filters.length)
    ) {
      ...
      return msg.channel.send(
        `Invalid filter. Available filters: ${Object.keys(client.distube.filters).join(
          ', ',
        )} (use "none" or "off to remove filter)`,
      );
    }
